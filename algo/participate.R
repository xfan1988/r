...
#Project volume as 1000 shares per minute

set.seed(200)
V_M = cumsum(c(0,100*(5+round(runif(65)*10))))

v_m = 1000          #Projected per-minute volume
V_P = N * cumsum(c(0, rep(v_m, 60)))/sum(c(0, rep(v_m, 60))) #Depends on Volume Profile
...
genChildOrder = function(strategy="POV", n=10){
  ...
  for (i in 1:n){
   ...
   U_pct = (((r+u_pct)*(V))-K[i])/(1-(r+u_pct))
   U_sh  = (((r)*(V))-K[i])/(1-r) + u_sh
   l[i] = min(q1, U_pct, U_sh, N-K[i])
   a[i] = lmda * l[i]
   filled[i] = 100*round(a[i] * runif(1) * 0.01)
   K[i] = K[i] + filled[i]
   rpov = K[i]/(V_M[i+1])
...
#TODO: Add dark fills
#Tighter to target (adjust share band)
